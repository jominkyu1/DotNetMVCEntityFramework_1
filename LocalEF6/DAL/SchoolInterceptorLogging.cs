...
namespace LocalEF6.DAL
{
    public class SchoolInterceptorLogging : DbCommandInterceptor
    {
        ...
        public override void ScalarExecuted(DbCommand command, DbCommandInterceptionContext<object> interceptionContext)
        {
            ...
            if (interceptionContext.Exception != null)
            {
                _logger.Error(interceptionContext.Exception, 
                    "[-ScalarExecuted-] Error executing command: {0}", command.CommandText);
            }
            else
            {
        ...
        public override void NonQueryExecuted(DbCommand command, DbCommandInterceptionContext<int> interceptionContext)
        {
            ...
            {
                _logger.Error(interceptionContext.Exception, 
                    "[-NonQueryExecuted-] Error executing command: {0}", command.CommandText);
            }
            else
            {
        ...
        public override void ReaderExecuted(DbCommand command, DbCommandInterceptionContext<DbDataReader> interceptionContext)
        {
            ...
            {
                _logger.Error(interceptionContext.Exception, 
                    "[-ReaderExecuted-] Error executing command: {0}", command.CommandText);
            }
            else
            {
